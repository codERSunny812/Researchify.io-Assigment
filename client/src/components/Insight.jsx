...
const Insight = () => {
  ...
  const sendMessage = async () => {
    ...
    const response = await fetch(
      `${import.meta.env.VITE_BACKEND_URI}/api/chat/message`,
      {
        method: "POST",
  ...
  return (
    <div
      className={`border bg-white px-4 sm:px-6 py-2 rounded-3xl shadow-md flex flex-col gap-2 transition-all duration-300 md:pt-6
            ${isMaximized ? "w-full h-5/6 justify-between" : "h-fit"}`}
    >
      {/* Header */}
      ...
      <div className={`flex flex-col ${isMaximized ? "gap-4" : "gap-2"}`}>
        {conversation.length > 0 ? (
          <div
            ...
            className={`overflow-auto p-2 border my-2 flex flex-col gap-2 transition-all duration-300 
              ${isMaximized ? "h-[calc(100vh-300px)]" : "h-44"}`}
          >
            {conversation.map((msg, index) => (
              ...
              >
                <div
                  className={`px-4 py-2 text-sm sm:text-base max-w-xs text-white 
                    ${msg.role === "user" ? "bg-green-500 rounded-l-2xl rounded-br-2xl" : "bg-gray-500 rounded-r-2xl rounded-bl-2xl"}`}
                >
                  {msg.content}
                </div>
              </div>
          ...
          <>
            <h1 className="text-xl sm:text-2xl font-semibold text-green-600 capitalize text-center sm:text-left md:mt-5">
              Hi, {userName}.
            </h1>
            <p className="text-green-600 text-lg sm:text-xl text-center sm:text-left">How can I help you?</p>

            {/* Insight Options */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-1">
              {obj.map(({ title, icon, id }) => (
                <div key={id} className="md:p-3  p-1 border rounded-lg text-center sm:text-left">
        ...
        <div className="flex items-center gap-2 ">
          <input
            ...
            className="flex-grow p-2 outline-none border rounded-lg text-sm sm:text-base md-w-auto w-1/2"
            value={message}
